...
const ConnectPage = () => {
  ...
  useEffect(() => {
    if (isConnected && chainId !== sepolia.id) {
      log('Switching to Sepolia network')
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setAuthState(prev => ({ ...prev, error: 'Failed to connect wallet. Please try again.' }))
    } finally {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      setAuthState(prev => ({ ...prev, error: 'Signing failed. Please try again.' }))
    } finally {
  ...
  const handleUsernameSubmit = async () => {
    ...
    try {
      ...
      if (error.message?.includes('username') || error.message?.includes('taken') || error.message?.includes('exists')) {
        errorMessage = 'Username is already taken. Please try another.'
      } else if (error.message?.includes('address') || error.message?.includes('wallet')) {
        errorMessage = 'This wallet address is already registered. Please use a different address.'
      } else {
  ...
  const getTitle = () => {
    switch (authState.step) {
      ...
      case 'verify':
        return authState.existingUser ? 'Welcome Back, Explorer!' : 'Verifying Your Account'
      case 'username':
        return 'Choose Your Username'
      case 'complete':
        return authState.existingUser ? 'Welcome Back to Bloxland!' : 'Welcome to Bloxland!'
      default:
        return 'Connect Your Wallet'
    }
  }
  ...
  const getButtonText = () => {
    ...
    if (authState.isLoading && authState.step === 'verify') return 'Verifying account...'
    if (authState.isLoading && authState.step === 'username') return 'Creating account...'
  ...
  const getEnsDisplayName = () => {
    return authState.existingUser?.subDomainName ??
  ...
  return (
    <div className="connect-page-bg min-h-[100dvh] w-full text-white overflow-x-hidden flex items-center justify-center">
      <div className="text-center space-y-8 px-6 sm:px-8">
        ...
        {isConnected && authState.step !== 'complete' && authState.step !== 'username' && (
              ...
              {authState.username && !authState.error && (
                <div className="bg-green-500/20 border border-green-500/50 rounded-lg p-3">
                  <p className="text-green-300 text-sm">
                    📍 Your ENS: <span className="font-mono">{authState.username}.bloxland.eth</span>
                  </p>
                </div>
              )}
        ...
        <div className="flex justify-center space-x-2 mt-6">
          {['connect', 'sign', 'verify', 'username', 'complete'].map((step, index) => (
            <div
              key={step}
              className={`w-2 h-2 rounded-full transition-all duration-300 ${step === authState.step
                ? 'bg-blue-400 scale-125'
                : ['connect', 'sign', 'verify', 'username', 'complete'].indexOf(authState.step) > index
                  ? 'bg-green-400'
                  : 'bg-gray-600'
                }`}
            />
